...
public interface AlbumRepository extends JpaRepository<Album, Long> {
    ...
    @Query(
        value = "select distinct album from Album album left join fetch album.artist",
        countQuery = "select count(distinct album) from Album album"
    )
    Page<Album> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct album from Album album left join fetch album.artist")
    List<Album> findAllWithToOneRelationships();

    @Query("select album from Album album left join fetch album.artist where album.id =:id")
